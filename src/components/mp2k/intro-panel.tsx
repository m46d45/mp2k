...
function InvalidX(props: { cx?: number; cy?: number; size?: number }) {
  const { cx = 0, cy = 0, size = 8 } = props;
  const s = size;
  return (
    <g>
      <line x1={cx - s} y1={cy - s} x2={cx + s} y2={cy + s} stroke={POINT} strokeWidth={2.25} strokeLinecap="round" />
      <line x1={cx + s} y1={cy - s} x2={cx - s} y2={cy + s} stroke={POINT} strokeWidth={2.25} strokeLinecap="round" />
    </g>
  );
}
...
export function IntroPanel({ onOpenLab }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <nav aria-label="Tiga kurva" className="grid grid-cols-3 gap-1 rounded-[var(--radius-md)] border border-border bg-elevated p-1">
        {INTRO_CURVES.map((c) => {
      ...
      {curve === "little" && (
        <LittleLesson seen={done.little} onSee={(id) => mark("little", id)} onNext={() => setCurve("kingman")} />
      )}
      {curve === "kingman" && (
        <KingmanLesson seen={done.kingman} onSee={(id) => mark("kingman", id)} onNext={() => setCurve("inventory")} />
      )}
      {curve === "inventory" && (
        <InventoryLesson seen={done.inventory} onSee={(id) => mark("inventory", id)} />
      )}

      {allFormulas ? (
        <BridgeCard onOpenLab={onOpenLab} />
      ) : (
        <p className="text-xs text-faint">Setelah ketiga skenario dijelajahi, jembatan ke kasus muncul di sini.</p>
      )}
    </div>
  );
}
...
function formulaReady(id: IntroCurve, seen: string[]): boolean {
  const need = id === "little" ? LITTLE_SCENARIOS : id === "kingman" ? KINGMAN_SCENARIOS : INV_SCENARIOS;
      ...
      <div className="h-[280px] w-full sm:h-[320px]">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={line} margin={{ top: 12, right: 16, left: 4, bottom: 20 }}>
            ...
            <XAxis dataKey="wip" type="number" domain={[0, 20]} tick={{ fill: MUTED, fontSize: 11 }} label={{ value: "WIP", position: "insideBottom", offset: -8, fill: MUTED, fontSize: 12 }} />
            <YAxis domain={[0, 10]} tick={{ fill: MUTED, fontSize: 11 }} label={{ value: "CT", angle: -90, position: "insideLeft", fill: MUTED, fontSize: 11 }} />
            <Tooltip formatter={(v: number) => formatNum(v)} />
            <Line data={baseLine} type="linear" dataKey="ct" name="TH = 2" stroke="#c4c4c4" strokeDasharray="5 4" strokeWidth={1.5} dot={false} isAnimationActive={false} />
            <Line type="linear" dataKey="ct" name={`TH = ${formatNum(point.th)}`} stroke={POINT} strokeWidth={2.25} dot={false} isAnimationActive={false} />
            <ReferenceDot x={point.wip} y={point.ct} r={7} fill={POINT} stroke="#fff" strokeWidth={2} />
            {showBaseDot ? (
              <ReferenceDot x={LITTLE_BASE.wip} y={LITTLE_BASE.ct} r={6} fill="#fff" stroke={POINT} strokeWidth={1.75} />
            ) : null}
            {broken ? (
              <ReferenceDot x={LITTLE_ATTEMPT.wip} y={LITTLE_ATTEMPT.ct} r={0} shape={<InvalidX size={8} />} />
            ) : null}
          </ComposedChart>
        </ResponsiveContainer>
      </div>
...
function KingmanLesson({
  ...
  return (
      ...
      formula={
        <div className="space-y-3">
          ...
          <div className="rounded border border-border/60 bg-surface px-3 py-2">
            <p className="text-[11px] font-medium uppercase tracking-wide text-faint mb-1.5">Pengganda utilisasi ū/(1−ū)</p>
            <div className="grid grid-cols-3 gap-2 text-center font-mono text-sm">
              <div>
                <p className="text-faint text-[11px]">ū = 0,80</p>
                <p className="font-semibold text-fg text-base">4</p>
              </div>
              <div>
                <p className="text-faint text-[11px]">ū = 0,85</p>
                <p className="font-semibold text-fg text-base">≈ 5,7</p>
              </div>
              <div>
                <p className="text-faint text-[11px]">ū = 0,90</p>
                <p className="font-semibold text-fg text-base">9</p>
              </div>
            </div>
    ...
    >
      ...
      <IdentityRow items={[["ū", end.u, true], ["V", end.v], ["CT/te", to.ratio]]} />

      <div className="h-[280px] w-full sm:h-[320px]">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chart.series} margin={{ top: 12, right: 12, left: 4, bottom: 20 }}>
            <CartesianGrid stroke={GRID} strokeDasharray="3 3" />
            <XAxis dataKey="u" type="number" domain={[0, 1]} ticks={[0, 0.2, 0.4, 0.6, 0.8, 1]} tick={{ fill: MUTED, fontSize: 11 }} label={{ value: "ū", position: "insideBottom", offset: -8, fill: MUTED, fontSize: 12 }} />
            <YAxis domain={[0, KINGMAN_Y_MAX]} tick={{ fill: MUTED, fontSize: 11 }} label={{ value: "CT/te", angle: -90, position: "insideLeft", fill: MUTED, fontSize: 11 }} />
            <Tooltip />
            ...
            {sc ? <ReferenceDot x={from.u} y={from.ratio} r={6} fill="#fff" stroke={POINT} strokeWidth={1.75} /> : null}
...
function InventoryLesson({ seen, onSee }: { seen: string[]; onSee: (id: string) => void }) {
  const [active, setActive] = useState<string | null>(null);
  ...
  const curveNow = useMemo(() => invCurve(apply.leadTime, apply.demandCv), [apply.leadTime, apply.demandCv]);
  const curveBase = useMemo(() => invCurve(INV_BASE.leadTime, INV_BASE.demandCv), []);
  ...
  return (
    ...
    >
      ...
      <IdentityRow items={[["Ī", pt.inv], ["FR", pt.fr / 100, true]]} />

      <div className="h-[280px] w-full sm:h-[320px]">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={curveNow} margin={{ top: 12, right: 16, left: 4, bottom: 20 }}>
            <CartesianGrid stroke={GRID} strokeDasharray="3 3" />
            <XAxis dataKey="fr" type="number" domain={[50, 100]} tick={{ fill: MUTED, fontSize: 11 }} label={{ value: "FR (%)", position: "insideBottom", offset: -8, fill: MUTED, fontSize: 12 }} />
            <YAxis dataKey="inv" type="number" domain={[0, "auto"]} tick={{ fill: MUTED, fontSize: 11 }} label={{ value: "Inventory", angle: -90, position: "insideLeft", fill: MUTED, fontSize: 11 }} />
            <Tooltip formatter={(value: number) => [formatNum(value), "Inventory"]} labelFormatter={(fr: number) => `FR ${formatNum(fr)}%`} />
            {sameStock ? (
              <Line data={curveBase} type="monotone" dataKey="inv" name="semula" stroke="#c4c4c4" strokeDasharray="5 4" strokeWidth={1.5} dot={false} isAnimationActive={false} />
            ) : null}
            <Line type="monotone" dataKey="inv" name="kurva" stroke={POINT} strokeWidth={2.25} dot={false} isAnimationActive={false} />
            {sc?.id === "I1"
              ? steps.map((s, i) => (
                  <ReferenceDot
                    key={INV_Z_STEPS[i]}
              ...
              : (
                <>
                  {showBase ? (
                    <ReferenceDot x={basePt.fr} y={basePt.inv} r={6} fill="#fff" stroke={POINT} strokeWidth={1.75} />
                  ) : null}
                  <ReferenceDot x={pt.fr} y={pt.inv} r={7} fill={POINT} stroke="#fff" strokeWidth={2} />
                  {sameStock ? <ReferenceLine y={pt.inv} stroke="#a3a3a3" strokeDasharray="4 3" /> : null}
                </>
              )}
          </ComposedChart>
        </ResponsiveContainer>
  ...
  return (
    <div className="space-y-4 rounded-[var(--radius-xl)] border border-border bg-surface p-5">
      ...
      {ready && onNext && nextLabel ? (
        <button type="button" onClick={onNext} className="inline-flex min-h-11 items-center gap-2 rounded-[var(--radius-sm)] bg-primary px-4 text-sm font-medium text-primary-fg">
          {nextLabel}
      ...
      <div className={cn("grid gap-2", items.length === 2 ? "grid-cols-2" : "grid-cols-3")}>
        {items.map(([k, v, pct]) => (
          <div key={k} className="rounded-[var(--radius-sm)] border border-border bg-elevated px-3 py-2">
            ...
            <p className="font-mono text-xl font-semibold tabular-nums text-fg">{pct ? formatPct(v) : formatNum(v)}</p>
          </div>
        ))}
      </div>
      {product ? (
...
function Say({ children, valid }: { children: ReactNode; valid: boolean }) {
  return (
    <p className={cn("rounded-[var(--radius-sm)] border px-3 py-2.5 text-sm leading-relaxed", valid ? "border-border bg-elevated text-fg" : "border-fg bg-elevated text-fg")}>
      {children}
    </p>
  );
}
...
function BridgeCard({ onOpenLab }: { onOpenLab: () => void }) {
  return (
    <div className="rounded-[var(--radius-xl)] border border-border bg-surface p-5">
      ...
      <button type="button" onClick={onOpenLab} className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-[var(--radius-sm)] bg-primary px-4 text-sm font-medium text-primary-fg">
        Lanjut ke kasus
        <ArrowRight className="size-4" />
      </button>
    </div>
