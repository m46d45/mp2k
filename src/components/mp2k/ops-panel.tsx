      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-muted leading-relaxed">
            ...
            {" "}(notasi &amp; warna sama dengan kurva). Alur: Run DES → <strong className="font-medium text-fg">Isi dari DES</strong> →{" "}
            <strong className="font-medium text-fg">Mulai perhitungan</strong>. Marker oranye di kurva = posisi DES.
          </p>
        </div>
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center gap-4 py-12 text-center">
            <p className="max-w-md text-sm text-muted leading-relaxed">
              Setelah strip DES terisi, tekan <strong className="text-fg">Mulai perhitungan</strong> untuk
              menampilkan tiga grafik. Marker oranye = titik operasi empiris DES.
            </p>
    ...
    <div className="space-y-3">
      ...
      {desPoint.ready ? (
        <p className="text-[11px] leading-relaxed" style={{ color: DES_METRIC_COLORS.des }}>
          ...
          Little: WIP ≈ TH×CT pada sistem stabil.
        </p>
      ) : null}
    </div>
  );
  ...
  return (
    <div className="space-y-3">
      <p className="font-mono text-xs text-muted">
        {op.name}: te={formatNum(op.te)} · ū={formatPct(kingman.u)} · V={formatNum(kingmanChart.Vop)} · CT/te={formatNum(kingmanChart.point.ctRatio)}
      </p>
      <div className="h-[360px] w-full">
        <ResponsiveContainer width="100%" height={360}>
      ...
      {desPoint.ready ? (
        <p className="text-[11px] leading-relaxed" style={{ color: DES_METRIC_COLORS.des }}>
          Marker oranye DES: ū_bot={formatPct(desPoint.utilBot)} ({desPoint.bottleneck}), CT/te≈{formatNum(desPoint.ctRatioBot)}.
          CT sistem multi-moda ≠ CT bottleneck murni — selisih adalah bahan diskusi.
        </p>
      ) : null}
    </div>
  );
...
function InventoryChart({ snap, desPoint }: { snap: CalcSnapshot; desPoint: DesPt }) {
  ...
  return (
    <div className="space-y-3">
      <p className="font-mono text-xs text-muted">
        FR teori={formatPct(inv.fillRate)} · Ī={formatNum(inv.avgInventory)}
      </p>
      <div className="h-[360px] w-full">
        <ResponsiveContainer width="100%" height={360}>
      ...
      {desPoint.ready ? (
        <p className="text-[11px] leading-relaxed" style={{ color: DES_METRIC_COLORS.des }}>
          Marker oranye DES: FR={formatPct(desPoint.fillRate)}. Jika lebih rendah dari teori, cek stockout, L, buffer, CONWIP di Simulasi.
        </p>
      ) : null}
    </div>
  );
